...
def main():
  ...
  def sinusoidal_positional_encoding(L=10):
    seq = torch.arange(L) * torch.pi

    def helper(x):
      if len(x.shape) == 2:
        pos_enc = torch.zeros((x.shape[0], 1 + 2 * L, x.shape[1]))
        pos_enc[:, 0, :] = x
        ...
        pos_enc = torch.zeros((x.shape[0], x.shape[1], 1 + 2 * L, x.shape[2]))
        pos_enc[:, :, 0, :] = x
  ...
  def train(model, train_loader, loss_fn, optimizer, img, epochs=10):
    inputs = torch.stack(torch.meshgrid(torch.arange(IMAGE_HEIGHT) / IMAGE_HEIGHT, torch.arange(IMAGE_WIDTH) / IMAGE_WIDTH), dim=-1).reshape(-1, 2)
  ...
  def pixel_to_ray(K, c2w, uv):
    ...
    x_w = transform(c2w, torch.cat((pixel_to_camera(K, uv, 1.0), torch.ones_like(uv[:, 0]).unsqueeze(1)), dim=1).unsqueeze(2)).squeeze(2)[:, :3]
  ...
  class NeRF3DImageDataset(Dataset):
    ...
    def get_vals(self, vals):
      image_idx = vals // (self.images.shape[1] * self.images.shape[2])
      x = vals % self.images.shape[2]
      y = (vals // self.images.shape[2]) % self.images.shape[1]
      pixels = self.images[image_idx, y, x, :]
  ...
  loaded = np.load('lego_200x200.npz')
  c2ws_train = torch.Tensor(loaded['c2ws_train'])

  def sample_points_from_ray(r_o, r_d, near=2.0, far=6.0, num_samples=64, perturb=True):
    intervals = torch.linspace(0.0, 1.0, num_samples + 1).repeat(r_o.shape[0], 1)
    if perturb:
      intervals = torch.clamp(intervals + torch.rand_like(intervals) / num_samples, torch.zeros_like(intervals), torch.ones_like(intervals))
  ...
  K = torch.Tensor(np.array([[loaded['focal'], 0, IMAGE_WIDTH // 2], [0, loaded['focal'], IMAGE_HEIGHT // 2], [0, 0, 1]]))
  ...
  def volumetric_rendering(sigmas, rgbs, deltas):
    ...
    T = torch.zeros((probs.shape[0], probs.shape[1] + 1, probs.shape[2]))
    T[:, 0, :] = 1.0
      ...
      with torch.no_grad():
        ...
        if (epoch + 1) % 100 == 0:
          for i in range(10):
            ...
            y_pred = y_pred * 255.0
            y_pred = y_pred.astype(np.uint8)
            plt.imsave('outputs/nerf_mlp/{}_{}.png'.format(epoch, i), y_pred, format='png')
    return losses, psnrs
  
  torch.manual_seed(42)
  sigmas = torch.rand((10, 64, 1))
  rgbs = torch.rand((10, 64, 3))
  step_size = torch.ones((10, 64)) * (6.0 - 2.0) / 64
  ...
  dataset = NeRF3DImageDataset(loaded['images_train'], c2ws_train, K, loaded['focal'])
  validation_dataset = NeRF3DImageDataset(loaded['images_val'], torch.Tensor(loaded['c2ws_val']), K, loaded['focal'])
  model = NeRFMLPModel(10)
  loss_fn = nn.MSELoss()
  optimizer = torch.optim.Adam(model.parameters(), lr=4e-3)
  ...
  plt.ylabel('Loss')
  plt.plot(losses)
  plt.savefig('outputs/nerf_mlp/loss.png')
  plt.clf()
  plt.title('PSNR over time')
  plt.xlabel('Iterations')
  plt.ylabel('PSNR')
  plt.plot(psnrs)
  plt.savefig('outputs/nerf_mlp/psnr.png')

  print("Evaluating test set...")
  test_dataset = NeRF3DImageDataset(torch.zeros((100, 200, 200, 3)), torch.Tensor(loaded['c2ws_test']), K, loaded['focal'])
  with torch.no_grad():
    for i in range(60):
      ...
      y_pred = y_pred * 255.0
      y_pred = y_pred.astype(np.uint8)
      plt.imsave('outputs/nerf_mlp/test_{}.png'.format(i), y_pred, format='png')
